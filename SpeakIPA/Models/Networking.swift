...
struct APIRequest {
    static func getTranscription(language: String, my_word:String, completion: @escaping (String?, String?) -> Void) {
        let appId = "a607e7f6"
        ...
        session.dataTask(with: request, completionHandler: { data, response, error in
            if let _ = response,
                ...
                audioFile = pronunciation!["audioFile"] as! String
                return completion(transcription, audioFile)
            } else {
                print(error!)
